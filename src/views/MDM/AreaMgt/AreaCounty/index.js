...
import { GET_COUNTY_DATA, ADD_COUNTY, EDIT_COUNTY, DEL_COUNTY } from './AreaCountyQuery'
//###################################################//
...
const CountyAreaView = ({ COUNTY_URP_ID }) => {
  ...
  const { data: LOAD_COUNTY_DATA, refetch } = useQuery(GET_COUNTY_DATA, {
    // 데이터 삭제시 그리드 테이블 리프레쉬
    fetchPolicy: 'network-only',
    skip: COUNTY_URP_ID === undefined,
    variables: { param: { AREA_TP_CD: '003', UPR_AREA_ID: COUNTY_URP_ID }, ver },
  })

  const [addMutation] = useMutation(ADD_COUNTY, {
    //신규 추가 뮤테이션
    variables: {
      param: { ...DTL_DATA, UPR_AREA_ID: COUNTY_URP_ID, AREA_TP_CD: '003' },
      ver,
    },
  })
  const [editMutation] = useMutation(EDIT_COUNTY, {
    //데이터 수정 뮤테이션
    variables: {
      param: { ...DTL_DATA, UPR_AREA_ID: COUNTY_URP_ID, AREA_TP_CD: '003' },
      ver,
  ...
  const [delMutation] = useMutation(DEL_COUNTY, { variables: { param: { AREA_IDS: UI_INFO.DEL_AREA_ID }, ver } })
  //#############################################################//
  ...
  useEffect(() => {
    ...
    if (COUNTY_URP_ID === undefined) {
      resetDtl()
      resetInfo()
    }

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [UI_INFO.SEL_FOCUS, GRID_API, COUNTY_URP_ID, DTL_DATA.AREA_ID, LOAD_COUNTY_DATA])
  //#################################################//
  ...
  const onSubmitDtl = async datas => {
    ...
    if (UI_INFO?.ADD_FLAG === true) {
      //신규 subMit 실행 함수
      if (LOAD_COUNTY_DATA?.getArea?.map(item => item.AREA_ID).some(id => id === datas.AREA_ID)) {
        ...
        <Box mt={1}>
          <Card>
            ...
            <Box className={classes.TopBarBox}>
              <div>
                <h3>
                  총{LOAD_COUNTY_DATA?.getArea?.length > 0 ? <span>{LOAD_COUNTY_DATA?.getArea?.length}</span> : <span>0</span>}건
                </h3>
              </div>
              <div>
                <Button
            ...
            <Box>
              <div className="ag-theme-alpine" style={{ position: 'relative', height: '575px', width: '100%' }}>
                <AgGridReact
                  ...
                  rowData={LOAD_COUNTY_DATA?.getArea || []}
                  defaultColDef={defaultColDef}
                  localeText={{ noRowsToShow: '시도 코드를 클릭해주세요.' }}
...
export default React.memo(CountyAreaView)

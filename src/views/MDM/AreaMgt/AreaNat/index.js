...
//############# APOLLO IMPORT #######################//
import { GET_AREA_DATA, GET_CTNT_CD, ADD_AREA, EDIT_AREA, DEL_AREA } from './AreaNatQuery'
...
const NatAreaView = ({ SET_URP_CD }) => {
  ...
  const resetDtl = () => {
    SET_DTL_DATA(props => ({ ...props, AREA_ID: '', AREA_NM: '', CTNT_CD: '' }))
    SET_URP_CD(props => ({ ...props, STT_URP_ID: undefined, COUNTY_URP_ID: undefined }))
    reset()
  }
  //######################################//
  ...
  const [LOAD_NAT, { data: LOAD_NAT_DATA }] = useLazyQuery(GET_AREA_DATA, {
    variables: { param: { ...SRCH_DATA, AREA_TP_CD: '001' }, ver },
    fetchPolicy: 'network-only',
  })
  const [addMutation] = useMutation(ADD_AREA, {
    //신규 추가 뮤테이션
    variables: {
      param: { ...DTL_DATA, AREA_TP_CD: '001' },
      ver,
    },
  })
  const [editMutation] = useMutation(EDIT_AREA, {
    //데이터 수정 뮤테이션
    variables: {
      param: { ...DTL_DATA, AREA_TP_CD: '001' },
      ver,
  ...
  const [delMutation] = useMutation(DEL_AREA, { variables: { param: { AREA_IDS: UI_INFO.DEL_AREA_ID }, ver } })
  //#############################################################//
  ...
  }, [UI_INFO.SEL_FOCUS, GRID_API, LOAD_NAT, DTL_DATA.AREA_ID, LOAD_NAT_DATA])
  //#################################################//

  //##############  SRCH_EVENT   ##################//
  /**
   * 조회시 인풋 값을 받아와 setParam저장시켜 LOAD_NAT useQuery 실행 시킴.
  ...
  const onRowClickd = ev => {
    ...
    SET_DTL_DATA(props => ({ ...props, CTNT_CD: ev?.data?.CTNT_CD }))
    SET_URP_CD(props => ({ ...props, STT_URP_ID: ev?.data?.AREA_ID, COUNTY_URP_ID: undefined }))
    resetInfo()
  }
  ...
  const editClick = () => {
    if (getValues('AREA_ID') !== '') {
      ...
      toast.error(`수정을 원하는 국가를 클릭해주세요.`)
    }
  }
        ...
        <Box mt={1}>
          <Card>
            ...
            <Box className={classes.TopBarBox}>
              <div>
                <h3>총{LOAD_NAT_DATA?.getArea?.length > 0 ? <span>{LOAD_NAT_DATA?.getArea?.length}</span> : <span>0</span>}건</h3>
              </div>
              <div>
                <Button
                ...
                <Grid container spacing={2}>
                  <Grid item lg={6} xs={12}>
                    <TextField
                      ...
                      helperText={validate(errors?.AREA_ID, getValues('AREA_ID')) ? '국가ID를 입력해주세요.' : false}
                      style={{ width: '100%' }}
                      label="국가 ID *"
                      fullWidth
                      variant="outlined"
                  ...
                  <Grid item lg={6} xs={12}>
                    <TextField
                      ...
                      helperText={validate(errors?.AREA_NM, getValues('AREA_NM')) ? '국가 명을 입력해주세요.' : false}
                      style={{ width: '100%' }}
                      label="국가 명 *"
                      fullWidth
                      variant="outlined"
                  ...
                  <Grid item lg={12} xs={12}>
                    <FormControl style={{ width: '100%' }} size="small" variant="outlined">
                      ...
                      <Select
                        label="대륙 코드"
                        value={DTL_DATA.CTNT_CD || ''}
                        onChange={ev => SET_DTL_DATA(props => ({ ...props, CTNT_CD: ev.target.value }))}
                      >
...
export default React.memo(NatAreaView)
